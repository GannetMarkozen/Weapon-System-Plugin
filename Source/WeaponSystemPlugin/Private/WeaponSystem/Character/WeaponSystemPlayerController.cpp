...
void FNetParams::Set(const UFunction* InFunction, const uint8* InParams)
{
	...
	TFunction<void(FProperty*,int32)> ThisLambda = nullptr;
	...
	if(HasParams())
	{
		...
		Memory = (uint8*)FMemory::Malloc(Function->ParmsSize);
		
		// Copy value of parameter from InParams to Memory
		auto CopyParam = [=](FProperty* Prop, const int32 Offset)
		{
...
void FNetParams::Invalidate()
{
	...
	if(Memory)
	{
		auto DestroyParam = [this](FProperty* Prop, const int32 Offset)
		{
			uint8* const MemoryPtr = Memory + Offset;
			if(const auto* StructProp = CastField<FStructProperty>(Prop))
...
bool FNetParams::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	...
	{
		auto NetSerializeParam = [&](FProperty* Prop, const int32 Offset)
		{
			auto* const MemoryPtr = Memory + Offset;
			if(const auto* StructProp = CastField<FStructProperty>(Prop))
			{
				const auto* ScriptStruct = StructProp->Struct;
		...
		ForEachParam(NetSerializeParam);
	}

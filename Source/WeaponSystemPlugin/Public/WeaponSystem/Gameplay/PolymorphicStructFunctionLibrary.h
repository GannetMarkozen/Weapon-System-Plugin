...
class WEAPONSYSTEMPLUGIN_API UPolyStructFunctionLibrary final : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
protected:
...
public:
	...
	// Ensures the OutStruct parameter will be a valid cast and then extracts the data into the output. Not a true cast as the memory is being copied but this syntactically may make more sense than "Extract"
	UFUNCTION(BlueprintCallable, CustomThunk, Meta = (CustomStructureParam = "OutStruct", ExpandEnumAsExecs = "OutPin", CompactNodeTitle = "CAST"), Category = "Weapon System Function Library|Polymorphic Struct")
	static void Cast(const FPolyStruct& PolyStruct, int32& OutStruct, EStructCastPin& OutPin);
	static void execCast(UObject* Context, FFrame& Stack, void* const RESULT_PARAM)
	{
